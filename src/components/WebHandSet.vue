<template>
  <article>
    <h1>webplayer</h1>
    <button @click="closeVideoPageFunc" class="power">
      <span>power</span>
    </button>
    <button class="central"><span>Directional</span></button>
    <button @click="volumeDecFunc" class="volume-dec">
      <span>volume-dec</span>
    </button>
    <button @click="showListFunc" class="home"><span>Home</span></button>
    <button @click="volumeIncFunc" class="volume-inc">
      <span>volume-inc</span>
    </button>
    <button @click="rewindFunc" class="rewind"><span>Rewind</span></button>
    <button @click="togglePlayFunc" class="playpause">
...
article {
  ...
  background:
    ...
    /* noise / texture */ radial-gradient(circle, #00000011 1.5%, #0000 2%) 50%
      50% / 11% 2.5%,
    radial-gradient(circle at 80% 10%, #ffffff07 1.75%, #fff0 2%) 11% 0% / 7.4%
      3.33%,
    ...
    radial-gradient(circle at 70% 80%, #00000013 2.28%, #fff0 3.33%) 5% 5.5% /
      3% 4.5%,
    radial-gradient(circle at 20% 20%, #ffffff07 2.66%, #fff0 3.66%) 3% 5% / 2%
      3%,
    radial-gradient(circle at 40% 70%, #00000012 2%, #fff0 3.4%) 5% 7.5% / 3%
      4.5%,
    radial-gradient(circle at 60% 30%, #ffffff05 2%, #fff0 3%) 3% 9.5% / 3.33%
      4.44%,
    /* base */ radial-gradient(130% 55% at 0 0%, #ccc3, #fff0),
  ...
  box-shadow: inset 0.2vmin -0.2vmin 0.5vmin #1a1915,
    inset 0.3vmin 0vmin 0.8vmin #fff2, inset -0.1vmin -0.2vmin 0.3vmin #1a1915,
    inset -0.2vmin 0vmin 3vmin #fff2, inset 0 0.3vmin 0.6vmin #000a,
    inset 0vmin 0.4vmin 0.8vmin #fff8, 2vmin 3vmin 3vmin #0002,
    1vmin 2vmin 2vmin #0002, 0 0 1vmin #0001, 1vmin 1vmin 1vmin #0001,
    4vmin 4vmin 5vmin #6662;
  ...
  * {
  ...
  h1 {
    ...
    background-image: linear-gradient(177deg, #afafaf56, #0002 50%),
      linear-gradient(#0009 0 0);
  ...
  button {
    ...
    --icon-color: #eee8;
    --icon-color-solid: rgb(238, 238, 238);
    ...
    overflow: hidden;
    cursor: pointer;
    background: var(--icon), linear-gradient(135deg, #2e2c2d00, #1e1c1d88),
      var(--bg);
    ...
    &:where(:nth-of-type(3n + 1)) {
      left: 50%;
    }

    &:where(:nth-of-type(3n + 2)) {
      left: 79%;
    }
    ...
    &.power {
      ...
      &::before {
        ...
        top: 0;
        left: 0;
        background: linear-gradient(var(--icon-color-solid) 0 0) 50% 35% / 4%
            12% no-repeat,
          radial-gradient(
              farthest-side at 50% 0,
              #0000 75%,
              var(--icon-color-solid) 0 95%,
              #0000 0
            )
            50% 63% / 40% 20% no-repeat;
    ...
    &.central {
      ...
      background: radial-gradient(at 50.1% 50.1%, #0008 40%, #0000 43%),
        /* shinny */
          radial-gradient(50% 50% at -0.75% 105%, #1b1b1b 52%, #0000 55%) 0% 55% /
          86.5% 40% no-repeat,
        radial-gradient(50% 50% at -0.75% 105%, #2f2d2c 52%, #0000 55%) 0% 85% /
          125% 35% no-repeat,
        ...
        /* shinny */ radial-gradient(at 48% 48%, #1d1d1d88 30%, #0000 40%),
        radial-gradient(at 52% 52%, #fff2 30%, #0000 40%),
        linear-gradient(135deg, #2e2c2d00, #1f1f1fcc),
        linear-gradient(135deg, #f8f8f819, #fff0),
        linear-gradient(135deg, #2a2925, #3e3c3d) #2e2c2d;
      box-shadow: 0 0 0.25vmin #000c, 0 0 0.1vmin 0.1vmin #0003,
        inset 0 0 0.25vmin #0008, inset 0.5vmin 0.5vmin 0.5vmin #fff1,
        inset -0.25vmin -0.25vmin 0.25vmin #0005,
        inset 0 -0.25vmin 0.75vmin -0.25vmin, inset -1vmin -0 0 -0.8vmin #fff3;
    }
    ...
    &.playpause {
      --icon: conic-gradient(
            at 100% 50%,
            #0000 240deg,
            var(--icon-color) 0 300deg,
            #0000 0
          )
          35.5% 49% / 20% 25% no-repeat,
        linear-gradient(
            90deg,
            var(--icon-color) 33%,
            #0000 0 67%,
    ...
    &.rewind {
      --icon: conic-gradient(
            at 0% 50%,
            #0000 60deg,
            var(--icon-color) 0 120deg,
            #0000 0
          )
          29% 49% / 20% 25% no-repeat,
        conic-gradient(
            at 0% 50%,
            #0000 60deg,
            var(--icon-color) 0 120deg,
    ...
    &.volume-dec {
      &::before {
        ...
        background: linear-gradient(var(--icon-color-solid) 0 0) 50% 50.75% /
          33% 4.25% no-repeat;
      }
    }
    ...
    &.home {
      &::before {
        ...
        left: 0;
        opacity: var(--icon-color-solid-opacity);
        background: linear-gradient(var(--icon-color-solid) 0 0) 50% 41.75% /
            33% 4.25% no-repeat,
          linear-gradient(var(--icon-color-solid) 0 0) 50% 51.75% / 33% 4.25%
            no-repeat,
          linear-gradient(var(--icon-color-solid) 0 0) 50% 61.75% / 33% 4.25%
            no-repeat;
      }
    }
    ...
    &.volume-inc {
      &::before {
        ...
        left: 0;
        opacity: var(--icon-color-solid-opacity);
        background: linear-gradient(var(--icon-color-solid) 0 0) 50% 51.75% /
            33% 4.25% no-repeat,
          linear-gradient(var(--icon-color-solid) 0 0) 50% 51% / 4.25% 33%
            no-repeat;
      }
    }
